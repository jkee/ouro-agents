    ...
    for upd in updates:
        ...
        if st.get("owner_id") is None:
            ...
            try:
                ...
                handle_chat_direct(chat_id, (
                    ...
                    "Упомяни, что раз в день ты запускаешь цикл эволюции — сам находишь "
                    "что улучшить и улучшаешь. Владелец может направлять эволюцию: "
                    "ставить долгосрочные цели, просить фокусироваться на мелочах, "
                    "менять направление — как угодно."
                ))
                handle_chat_direct(chat_id, (
                    ...
                    "2. Просмотри ARCHITECTURE.md — обнови если нужно.\n"
        ...
        if agent._busy:
            ...
            if agent._busy:
                ...
                _consciousness.pause()
                _trigger_msg_id = msg.get("message_id", 0)
                def _run_task_and_resume(cid, txt, img, trigger_mid):
                    try:
                        handle_chat_direct(cid, txt, img, reply_to_message_id=trigger_mid)
                    finally:
                        _consciousness.resume()
                _t = threading.Thread(
                    target=_run_task_and_resume,
                    args=(chat_id, final_text, _batched_image, _trigger_msg_id),
                    daemon=True,
                )
                try:
                    _t.start()
