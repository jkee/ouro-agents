...
class TelegramClient:
    ...
    def download_file_bytes(self, file_id: str, max_bytes: int = 25_000_000) -> Tuple[Optional[bytes], str]:
        """Download a file from Telegram and return (raw_bytes, file_path). Returns (None, "") on failure."""
        try:
            r = requests.get(f"{self.base}/getFile", params={"file_id": file_id}, timeout=10)
            r.raise_for_status()
            data = r.json()
            ...
            log.warning("Failed to download bytes for file_id=%s", file_id, exc_info=True)
            return None, ""
